...
with open("report.txt", "w", encoding="utf-8") as file:
    file.writelines([
           ...
           " ".join(map(str, positive_data)) +"\n",
           " ".join(map(str, negative_data)) +"\n",
            ...
            f"Количество четных элементов первого файла: {len([i for i in positive_data if i % 2 == 0])} \n",
            f"Количество нечетных элементов второго файла: {len([i for i in positive_data if i % 2 != 0])}\n"
            ])
